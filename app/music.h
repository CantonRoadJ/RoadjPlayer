...
#include "qmmp/include/qmmpplugincache_p.h"
#include <qmmp/include/visualfactory.h>
#include <QProcess>
#include <QTimer>
#include <qfileinfo.h>
#include <QThread>
...
public:
    ...
    void adjustVolume(bool isOpen);
    // Q_INVOKABLE void getNextPage();

    int changeMode();
    //should save with qsettings
    int playMode;
...
Q_SIGNALS:
    void songChange(int index,QPixmap headPic
    ...
    void updateSongList(QStringList nameList,
                        int listType);
    void getFileList(QFileInfoList infoList,int listType);
    void addSong(QString name,QString singer);
    ...
    void startTest();
    void eqDoneTest(QList<QPointF> m_points);
    void eqError(QList<QPointF> m_points);
    void eqVolumeChange(int volume);
